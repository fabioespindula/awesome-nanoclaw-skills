...
- Resolve vague words such as "this", "isso", "launch", "cliente", "arquitetura", or "plano" from the visible conversation when the target is clear.
...
- If the chat is about a marketplace launch and the user asks `/rethink Devemos lancar agora?`, treat the decision as launch timing for that marketplace.
- If the chat is about an enterprise customer requesting custom work and the user asks `/rethink aceito esse cliente?`, treat the decision as revenue now versus product focus.
- If the chat is about an updater default and the user asks `/rethink isso protege usuarios ou cria risco operacional?`, treat the decision as a technical and trust tradeoff.
